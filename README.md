...
Основной поток        | <ol><li>Администратор базы данных или историк ниндзя вносят информацию о клане: `name`, `village`, `prestige`</li><li>Создается клан с `name`, `village`, `prestige`</li></ol>    
 ...
 Прецедент              | Добавление новой битвы 
  ...
  Прецедент              | Разрушение скрытой деревни ниндзя 
   ...
   Основной поток        | <ol><li>Администратор базы данных или историк ниндзя обновляют статус ниндзя</li><li>Проверяется ранг погибшего ниндзя</li><li>Если ранг равен `Kage`, то выполняется прецедент "Выбор кандидатов на должность Каге"(ID 12)</li></ol>
   ...
   Основной поток        | <ol><li>Администратор базы данных или историк ниндзя обновляют информацию о главе клана.</li><li>Проверяется, является ли претендент членом клана</li><li>Если нет, вызывается исключение, запрещающее не членам клана являться его главой.</li><li>Если он является членом клана, проверяется его статус</li><li>Если статус равен `dead`, то вызывается исключение, запрещающее мертвым ниндзя назначаться новыми лидерами клана</li><li>Если статус `alive` то в таблицу `clan_leader` вносятся изменения</li></ol>
   ...
   Основной поток        | <ol><li>Администратор базы данных или историк ниндзя запрашивает время првления главы государства</li><li>Запрашивается дата начала</li><li>Запрашивается дата конца</li><li>Подсчитывается время правления (в днях)</li></ol>
   ...
   Основной поток        | <ol><li>Администратор базы данных или историк ниндзя запрашивают информацию о времени правления глав государств</li><li>запрашивается информация правлении о лидеров</li></ol>
...
### Индексы:
* `ninja_status` - b-tree индекс на атрибут `status` в таблице `Ninja`, позволяющий ускорить операции сравнения;
* `blord` - b-tree индекс на атрибут `beginning_of_regin` в таблице `country_lord`, позволяющий ускорить операции сравнения;
* `elord` - b-tree индекс на атрибут `end_of_regin` в таблице `country_lord`, позволяющий ускорить операции сравнения;
